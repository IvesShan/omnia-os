...
(async () => {
  ...
  // Extract window.__WPSENV__ and window.__CONFIG__
  const envData = await page.evaluate(() => {
    return {
      wpsenv: window.__WPSENV__ || null,
      config: window.__CONFIG__ || null,
      fileInfo: window.__WPSENV__ && window.__WPSENV__.file_info ? window.__WPSENV__.file_info : null
    };
  });

  fs.writeFileSync('/tmp/kdocs_env.json', JSON.stringify(envData, null, 2), 'utf-8');
  console.log(JSON.stringify({status:'ok', hasWpsenv: !!envData.wpsenv, hasConfig: !!envData.config}));
  await browser.close();
})();
