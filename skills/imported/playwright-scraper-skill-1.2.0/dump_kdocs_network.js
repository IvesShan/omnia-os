...
(async () => {
  ...
  page.on('response', async response => {
    ...
    if (url.includes('api') && (url.includes('record') || url.includes('sheet') || url.includes('table') || url.includes('data') || url.includes('dbs') || url.includes('database'))) {
      interestingUrls.push(url);
      try {
        const body = await response.text();
        fs.writeFileSync('/tmp/kdocs_api_' + interestingUrls.length + '.json', body, 'utf-8');
      } catch(e) {}
    }
  });
  ...
  fs.writeFileSync('/tmp/kdns_network_urls.txt', interestingUrls.join('\n'), 'utf-8');
  console.log(JSON.stringify({status:'ok', urls: interestingUrls}));
  await browser.close();
})();
