"""
tool_trigger.py — 工具触发判断模块

移植自 Flask 版 omnia/tool_trigger.py
适配 FastAPI 异步架构

核心功能：
1. 关键词匹配 + 上下文分析 → 判断是否需要工具
2. tool_choice 控制（required/auto/None）
3. 工具提示注入（对不支持 required 的 Provider）
4. 前置检查（git status 等高频场景自动注入）
"""
...
class ToolTriggerResult:
    """工具触发判断结果"""
    should_trigger: bool
    trigger_type: str  # keyword | context | explicit | promise | preroll | excluded
...
def get_tool_choice_for_provider(
    ...
    - 高置信度 + 支持 required 的 Provider → "required"
    - 中置信度 → "auto"
    - 不触发 → None
    """
