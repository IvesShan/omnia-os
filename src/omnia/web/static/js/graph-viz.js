...
const GraphViz = {
  ...
  initGraph() {
    ...
    // 创建 3d-force-graph 实例（工厂函数，不要用 new）
  ...
  getNodeSize(node) {
    // 预计算邻居数缓存
    if (!this._degreeCache) {
      this._degreeCache = {};
      this.graphData.links.forEach(l => {
        const sid = typeof l.source === 'object' ? l.source.id : l.source;
        const tid = typeof l.target === 'object' ? l.target.id : l.target;
        this._degreeCache[sid] = (this._degreeCache[sid] || 0) + 1;
        this._degreeCache[tid] = (this._degreeCache[tid] || 0) + 1;
      });
    }
    const connections = this._degreeCache[node.id] || 0;
