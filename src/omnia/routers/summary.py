...
@router.post("/chat/summarize/auto")
async def auto_summarize(
    ...
    当对话超过 20 条消息时，自动将前 15 条压缩成摘要，
    保留最近 5 条原始消息。
    """
    if not req.messages:
        return {"ok": True, "messages": [], "token_saved": 0}
    ...
    # 分割消息：旧消息（需要压缩）+ 新消息（保留原始）
    old_messages = req.messages[:-5]  # 压缩前 N-5 条
    new_messages = req.messages[-5:]  # 保留最后 5 条
    
    # 生成摘要
    summary_result = await summarize_conversation(
        SummarizeRequest(messages=old_messages, max_length=500),
        client
    )
    ...
    return {
        ...
        "token_saved": summary_result.token_saved
    }
