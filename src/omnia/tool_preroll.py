...
这样 LLM 在收到消息时已经包含了工具验证结果，无法跳过。

接入点：web_server.py 中 `/api/chat` 路由的 `assemble_wake_prompt()` 之后、`handle_chat()` 之前。
"""
...
def should_force_tool_check(user_message: str, last_assistant_message: str = "") -> bool:
    ...
    条件：
    1. 用户消息包含关键词触发器 → True
    2. 上一条助手消息包含"让我检查""让我看看"等 → True
    3. 用户消息包含"工具调用"相关词 → True
    """
    # 条件1：关键词触发器
